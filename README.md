...
現在、実際に比較できる対象は `.xlsx` のみです。`.xls` は変換層の入口だけ用意済みですが、実変換は未実装です。`.xlsm`、CSV、JSON、XMLには対応していません。
...
- `.xlsx または .xls ファイルを指定してください。`: 入力ファイルにはExcelファイルを指定してください。現時点で実際に比較できるのは `.xlsx` です。
