...
JSONの比較対象は値、キー、配列要素です。配列は初期版ではインデックスで比較します。
...
JSONの配列LCS対応、キー順の差分、JSON Lines、JSON5、スキーマ比較は将来検討とします。
...
- `JSONファイルの形式を読み取れません。`: JSON構文が壊れている可能性があります。カンマ、引用符、括弧の対応を確認してください。
- `JSONファイルをUTF-8として読み取れません。`: JSONをUTF-8で保存し直してから再実行してください。
