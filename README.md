...
2つの `.xlsx`、`.csv`、`.json`、`.xml` ファイルを比較し、比較結果をExcelファイルとして出力するGUIツールです。元ファイルは変更しません。
...
現在、実際に比較できる対象は `.xlsx` 同士、`.csv` 同士、`.json` 同士、または `.xml` 同士です。`.xls` は変換層の入口だけ用意済みですが、実変換は未実装です。`.xlsm` には対応していません。
...
- `.xlsx、.xls、.csv、.json、.xml のいずれかを指定してください。`: 入力ファイルには対応している拡張子のファイルを指定してください。現時点で実際に比較できるのは `.xlsx` 同士、`.csv` 同士、`.json` 同士、または `.xml` 同士です。
