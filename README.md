...
CSV比較では、CSVを `CSV` という1シートの表として扱い、結果は `.xlsx` で出力します。CSVの読み込み文字コードはUTF-8 BOM付き/なしに対応しています。
...
CSVの比較対象は各フィールドの文字列です。区切り文字はカンマ、文字コードはUTF-8として読み込みます。
...
- `CSVファイルをUTF-8として読み取れません。`: CSVをUTF-8で保存し直してから再実行してください。
