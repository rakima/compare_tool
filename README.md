...
2つの `.xlsx`、`.csv`、`.json` ファイルを比較し、比較結果をExcelファイルとして出力するGUIツールです。元ファイルは変更しません。
...
`Comparer` が形式非依存の比較戦略、`Difference` / `CompareResult` が共通結果モデルです。Excel固有処理は `ExcelReader`、`ExcelComparer`、`ExcelReportWriter`、CSV固有処理は `CsvReader`、`CsvComparer`、`CsvReportWriter`、JSON固有処理は `JsonReader`、`JsonComparer`、`JsonReportWriter` に分離しています。将来は同じインターフェースでXML用の実装を追加できます。GUIは `CompareUseCase` のみを呼び出します。
...
現在、実際に比較できる対象は `.xlsx` 同士、`.csv` 同士、または `.json` 同士です。`.xls` は変換層の入口だけ用意済みですが、実変換は未実装です。`.xlsm`、XMLには対応していません。

`.xlsx` と `.csv`、`.csv` と `.json` のように異なる形式同士の比較には対応していません。
...
XML対応は今後のリリースで追加予定です。初期版では、まず `.xml` 同士の比較に限定し、比較結果は他形式と同じく `.xlsx` で出力する想定です。
...
初期版では、コメント、処理命令、DTD/XSD検証、名前空間の高度な正規化、同名兄弟要素のキー指定比較、XML整形差分は対象外とします。

## よくあるエラー

- `旧ファイルと新ファイルに同じファイルが指定されています。`: 異なる2つのExcelファイルを指定してください。
- `.xlsx、.xls、.csv、.json のいずれかを指定してください。`: 入力ファイルには対応している拡張子のファイルを指定してください。現時点で実際に比較できるのは `.xlsx` 同士、`.csv` 同士、または `.json` 同士です。
