...
子要素は同じ内容の要素をLCSで対応付けます。途中に要素が追加・削除されても、後続要素がまとめて変更扱いになりにくい表示にしています。
...
XMLのコメント、処理命令、DTD/XSD検証、名前空間の高度な正規化、同名兄弟要素のキー指定比較、XML整形差分は対象外です。子要素の対応付けはLCSによる初期対応であり、`id` 属性などを使ったキー指定比較にはまだ対応していません。
