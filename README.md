...
2つの `.xlsx`、`.xls`、`.csv`、`.json`、`.xml` ファイルを比較し、比較結果をExcelファイルとして出力するGUIツールです。元ファイルは変更しません。
...
キー列は `A` または `A,C` のように列名だけを指定します。`A1` のようなセル座標やヘッダー名ではなく、1行目を見ながら列記号を入力してください。キー列入力欄は、比較方式で `キー列で比較` を選んだときだけ有効になります。キー値が重複している場合は、誤対応を避けるため比較を停止します。
...
入力Excelは `WorkbookPreparer` を通してから比較します。`.xlsx` はそのまま比較し、`.xls` はWindows版Excelを使って一時 `.xlsx` に変換してから既存のExcel比較処理へ渡します。
...
現在、実際に比較できる対象は `.xlsx` / `.xls` のExcel同士、`.csv` 同士、`.json` 同士、または `.xml` 同士です。`.xls` の比較にはWindows版Excelが必要です。`.xlsm` には対応していません。
...
- `.xlsx、.xls、.csv、.json、.xml のいずれかを指定してください。`: 入力ファイルには対応している拡張子のファイルを指定してください。現時点で実際に比較できるのは `.xlsx` / `.xls` のExcel同士、`.csv` 同士、`.json` 同士、または `.xml` 同士です。
...
- `キー列は列名だけを指定してください。`: `A1` やヘッダー名ではなく、`A` のように列記号だけを指定してください。
